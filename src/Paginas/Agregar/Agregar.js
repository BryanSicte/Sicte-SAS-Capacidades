...
const Agregar = ({ role }) => {
    ...
    const handleSelectTipoFacturacion = (item) => {
        ...
        if (item === 'ADMON') {
            setTipoMovilOptions(['AUXILIAR DE PROYECTO', 'BACKOFFICE', 'CONTRATISTA', 'COORDINADOR', 'DIBUJANTE', 'DISEÑADOR DE REDES HFC Y FIBRA', 'FORMADOR', 'GESTOR DE RUTA', 'LIDER DE ACOMETIDAS', 'LIDER DE DISEÑO', 'MOVIL DE APOYO (UNI/BIDI)', 'RESTRINGIDO', 'SUPERVISOR']);
        } else if (item === 'EVENTO') {
